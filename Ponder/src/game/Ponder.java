...
public class Ponder {
	...
	public static void initGraphics(final SwingGraphics window) {
		...
		theme[0][0] = new ImageIcon(SwingGraphics.scaleImage(tmp.getImage(), w,
				h));

		tmp = new ImageIcon(Ponder.class.getResource("/asterisk.png"));
		theme[1][0] = new ImageIcon(SwingGraphics.scaleImage(tmp.getImage(),
				w - 10, h - 10));

		tmp = new ImageIcon(Ponder.class.getResource("/ring.png"));
		theme[2][0] = new ImageIcon(SwingGraphics.scaleImage(tmp.getImage(),
				w - 10, h - 10));

		tmp = new ImageIcon(Ponder.class.getResource("/multip.png"));
		theme[3][0] = new ImageIcon(SwingGraphics.scaleImage(tmp.getImage(), w,
				h));

		tmp = new ImageIcon(Ponder.class.getResource("/Flag.png"));
		tmp = new ImageIcon(SwingGraphics.scaleImage(tmp.getImage(), w - 10,
				h - 15));
		theme[0][1] = tmp;
		theme[1][1] = tmp;
		theme[2][1] = tmp;
	...
	// BUG Turn doesn't end after player has spawned if all pieces were in the
	// spawn area
	...
	public static void main(String[] args) {
		...
		window.setClientele(new LocalPlayer(), new NetworkPlayer(), null); // window
																			// creates
																			// a
																			// default
																			// player
		...
		while (true) {
			try {
				while (logic.victor() == -1) {
					...
					while (logic.getCurrPlayer() != -1 && !window.loaded()
							&& !curr.turnOver(window, net))
						try {
					...
					if (logic.getCurrPlayer() != -1 && !window.loaded())
