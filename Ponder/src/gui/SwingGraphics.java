...
public class SwingGraphics {
	...
	public void printMove(List<Event> list) {
		for (Event e : list) {
			...
			if (e instanceof SpawnEvent) {
				...
				System.out.printf("%s %d's piece %s %s\n", sp.exiled ? "Exile" : "Spawn", sp.owner,
						sp.exiled ? "at" : "on", sp.pos);
			}
	...
	private void initialize() {
		...
		for (int i = 0; i != player_tags.length; ++i) {
			...
			for (int j = 0; j != 4; ++j) {
				JLabel flag_icon = new JLabel(new ImageIcon(
						SwingGraphics.class.getResource("/com/sun/java/swing/plaf/windows/icons/Computer.gif")));
		...
		newGame.setBounds(110, 145, 307, 54); // Don't know how to change it's
			...
			for (int x = 0; x != cells.length; ++x) {
				...
				cells[y][x].addMouseListener(new MouseAdapter() {
					private void updateMouseText(JButton cell) {
						...
						mseLbl.setText(out + "] P" + (logic.getCurrPlayer() + 1));// +
						// "
						// "
						// +
						// logic.turnOver());
					...
					public void mouseClicked(MouseEvent e) {
						...
						case MouseEvent.BUTTON1: // LEFT-CLICK
							...
							case PonderLogic.MOVE_CLICK:
								...
								if (logic.getPieceOwner(src) != -1)
									return; // Can't move to a tile where a
								// piece exists

								if (logic.canSlide(from, src)) {
									if (logic.hasMoved(from) || !logic.canCapFlag(from, src))
						...
						case MouseEvent.BUTTON2: // MIDDLE-CLICK
							...
							// for (int i = 0; i != 4; ++i) {
							// logic.addEvent(new
							// AttachEvent(logic.positionOf(src), i, false));
							// }
	 ...
	 * 
	 * @param icons
	 *            A 2d array filled by the following rules The four columns
	 *            correspond to the four players The columns are filled
	 *            accordingly 0 - Piece Icon 1 - Flag Icon 2 - Piece/Flag Icon
	 */
	...
	public static java.awt.Image scaleImage(java.awt.Image srcImg, int w, int h) {
		java.awt.image.BufferedImage resizedImg = new java.awt.image.BufferedImage(w, h,
	...
	/**
	 * Move a piece from one component to another (move/exile/spawn) Also
	 * performs necessary logic calls to update the game state
	 * 
	 * @param from
	 *            The component where the piece is currently located
	 * @param to
	 *            The component the piece is being moved to
	 */
	public void move(JComponent from, JComponent to) {
		if (from instanceof JButton) {
			...
			if (to instanceof JButton) {
				...
				logic.addPiece(t, logic.getPieceOwner(f)); // logic.getCurrPlayer()
				// should also work
				for (int i = 0; i != flags.length; ++i) { // Currently doesn't
					// allow for
					// dropable flags
					if (flags[i]) {
				...
				if (to != getStack(logic.getPieceOwner(f)))
			...
			logic.addPiece(t, logic.getCurrPlayer()); // Relies on current
			// player (Subject to
			// abuse depending on
			// how stack selection
			// works)
	...
	public void runEvent(Event e) {
		if (e instanceof MoveEvent) {
			...
			if (event.exiled) {
				...
				move(logic.getStack(), logic.getPiece(event.pos)); // Spawn the
				// piece
				logic.setStack(null);
				color(Color.BLACK);
			}
	...
	public boolean nonLocal() {
		return players[0] instanceof NetworkPlayer || players[1] instanceof NetworkPlayer
				|| players[2] instanceof NetworkPlayer || players[3] instanceof NetworkPlayer;
	}
}
